...
function createApiItem(api) {
    ...
    let currentEndpoint = api.endpoint; // Track the current endpoint
    ...
    */
    ...
    // Query input button (if you want to add it later)
    const queryInputButton = document.createElement('button');
    queryInputButton.dataset.query = ''; // Optional: store query input
    ...
    toggleResponseButton.onclick = async () => {
        ...
        if (isResponseVisible) {
            ...
            let queryFromButton = queryInputButton.dataset.query;

            if (!queryFromButton) {
                const userInput = prompt('Enter query or data (e.g., userId=123 or a full URL):');
                if (userInput) {
                    ...
                    if (isUrl) {
                        ...
                        endpointURL = endpointURL + connector + userInput;
                    }

                    currentEndpoint = endpointURL;
                    endpointSection.textContent = `Endpoint: ${endpointURL}`;
                    queryInputButton.dataset.query = userInput;
                } else {
