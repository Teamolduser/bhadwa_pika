const apiData = {
    "Downloader": [
        ...
        },
        {
            ...
            endpoint: "http://esm.api.olduser.tech/instagram/igdl2?apikey=anya-md&url="
        },
        {
            method: "GET",
...
function createApiItem(api) {
    ...
    let currentEndpoint = api.endpoint; // Start with the base endpoint
    ...
    endpointSection.textContent = `Endpoint: ${currentEndpoint}`;
    ...
    openButton.onclick = () => {
        window.open(currentEndpoint, '_blank');
    };
    ...
    copyButton.onclick = () => {
        navigator.clipboard.writeText(currentEndpoint)
            .then(() => alert('Copied: ' + currentEndpoint))
    ...
    */
    ...
    toggleResponseButton.onclick = async () => {
        ...
        if (isResponseVisible) {
            ...
            const userInput = prompt('Enter query (e.g. url=https://example.com or full URL):');
            if (!userInput) {
                alert('You must enter a query or URL.');
                toggleResponseButton.innerHTML = '<i class="fas fa-code"></i> Get Response';
                isResponseVisible = false;
                return;
            }

            const isFullUrl = userInput.startsWith('http') || userInput.startsWith('www');
            const newEndpoint = isFullUrl
                ? currentEndpoint + userInput : currentEndpoint + userInput;

            currentEndpoint = newEndpoint;
            endpointSection.textContent = `Endpoint: ${currentEndpoint}`;

            try {
                const response = await axios.get(currentEndpoint, { headers: { 'Accept': 'application/json, text/plain, */*', 'Accept-Language': 'en-US,en;q=0.9', 'X-Requested-With': 'XMLHttpRequest' } });
